...
\newcommand{\deadline}{11 December 2024, 13:15}
\newcommand{\hwNum}{2}
...
\thispagestyle{fancy}
  ...
  \begin{question}[18] Let $\mathcal{S}$ be a planar subdivision with
  ...
  \begin{question}[10]
    Let $P$ be a set of $n$ points in $\R^2$. The \emph{diameter}
    $\mathit{diam}(P)$ of $P$ is the maximum pairwise distance,
    $\max_{(p,q) \in P \times P} \|pq\|$. Prove that the points $p$
    and $q$ realizing the diameter are vertices of the convex hull
    $\mathit{CH}(P)$ of $P$.
  \end{question}

  \begin{question}[10]
    Let $C$ be a convex polygon whose vertices $c_1,..,c_n$ are given
    in clockwise order, i.e. you are given some pointer to an array
    storing the vertices of $C$ in that order. Give an algorithm that
    can test in $O(\log n)$ time if a query point $q \in \R^2$ lies
    inside $C$. Prove that your algorithm is correct and achieves the
    desired running time.
  \end{question}

  \begin{question}[7]
    Prove or disprove: The dual graph of any triangulation of a
    $y$-monotone polygon is always a chain, that is, any node in this
    graph has degree at most two.
  \end{question}
